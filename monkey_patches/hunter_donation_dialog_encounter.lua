local HunterDonationDialogEncounter = class()
local rng = _radiant.math.get_default_rng()
local LootTable = require 'stonehearth.lib.loot_table.loot_table'
local game_master_lib = require 'stonehearth.lib.game_master.game_master_lib'
local AceDonationDialogEncounter = require 'stonehearth_ace.monkey_patches.ace_donation_dialog_encounter'

function HunterDonationDialogEncounter:start(ctx, info)
   ...
   --Get the drop items
   local reward_items = {}
	if AceDonationDialogEncounter then
		if info.loot_table then
			self._sv.items = LootTable(info.loot_table)
									:roll_loot()
									
			local large_loot = nil
			if radiant.size(self._sv.items) > 11 then
				large_loot = true
			end
									
			for uri, detail in pairs(self._sv.items) do
				if large_loot and radiant.size(reward_items) == 10 then
					table.insert(reward_items, {count = nil, display_name = 'i18n(hunter:data.gm.campaigns.hunting.weekly_hunt.common.large_reward)' })
					break
				end
		...
		if info.loot_table then
			...
			for uri, quantity in pairs(self._sv.items) do
				...
				table.insert(reward_items, {count = quantity, display_name = display_name })
			end
		else
			self._sv.items = {}
		end
...
return HunterDonationDialogEncounter